...
#'
#' @examples
prepare_can_rev_exp <- function(canton_abb, category) {

  sheet <- case_when(category == "rev" ~ "einnahmen",
                     category == "exp" ~ "ausgaben",
                     category == "balance" ~ "bilanz")

  target_variables <- unlist(
    case_when(
      category == "rev" ~ list(c("Gesamteinnahmen", "Zinseinnahmen")),
      category == "exp" ~ list(c("Gesamtausgaben", "Zinsausgaben")),
      category == "balance" ~ list(c("Finanzvermögen",
                                     "Fremdkapital",
...
prepare_rev_exp <- function(df_cantons) {
  ...
  df_revenues <- map(
    df_cantons$canton,  ~prepare_can_rev_exp(canton_abb = .x, category = "rev")
    ) %>%
    bind_rows()
  ...
  df_expenditures <- map(
    df_cantons$canton, ~prepare_can_rev_exp(canton_abb = .x, category = "exp")
    ) %>%
    bind_rows()
...
prepare_balance <- function(df_cantons) {
  ...
  df_balance <- map(df_cantons$canton,
                    ~prepare_can_rev_exp(canton_abb = .x,
                                           category = "balance")) %>%
    bind_rows() %>%
    pivot_wider(values_from = "value", names_from = "item") %>%
    mutate(across(c(1, 3:7), ~if_else(is.na(as.numeric(.)), 0, as.numeric(.)))) %>%
    mutate(can_debt_net_mio = (Fremdkapital - Finanzvermögen) / 1000,    #
           can_debt_gross_mio = (`Laufende Verbindlichkeiten` +
                               `Kurzfristige Finanzverbindlichkeiten` +
                               `Langfristige Finanzverbindlichkeiten`) / 1000) %>%
    select(canton, year, can_debt_net_mio, can_debt_gross_mio)
